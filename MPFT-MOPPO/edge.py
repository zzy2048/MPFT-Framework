...
class EDGE:
    def __init__(self, environment, eval_env, num_objectives, lr=3e-4, seed=42, warm_up_steps=0, Xi=4000, Psi=1200,
                 edge_direction=0, buffer_size=512, pareto_ascent_num=2, opposite_num=1,
        ...
        self.max_train_steps = self.obj_episodes * buffer_size + 1e6  # 用于控制学习率衰减
        ...
        self.agent = ppo.PPO(self.env, self.eval_env, self.seed, num_objectives, max_train_steps=self.max_train_steps,
                             steps=buffer_size, use_seed=use_seed)
        self.max_action = float(self.env.action_space.high[0])
        self.state_dim = self.env.observation_space.shape[0]
        self.action_dim = self.env.action_space.shape[0]
        self.set_adam_eps = True
        self.use_memory = True  # Ant || Walk2D Ture else False
    ...
    def update_non_dominated_set(self, policy_optimizers):
        ...
        print(f" Pareto Edge {self.edge_direction + 1}, pareto policies size: {len(self.non_dominated_set)}")
    ...
    def update_ParetoFront(self, policy, optimizer):
        # 不包含目标 obj_id 的 Pareto 上升方向
        omega = self.agent.obtain_alpha_star(policy, optimizer, obj_id=self.edge_direction)
    ...
    def train(self):
        with tqdm(total=self.obj_episodes + self.pareto_front_size,
                  desc=f"Edge {self.edge_direction + 1} Training") as pbar:
            ...
            for i in range(self.obj_episodes):
                ...
                if i < self.warm_up and (i % 40 == 0):
                    r = self.reward_max / rewards
                    omega = ppo.getOmega(r)
                elif i >= self.warm_up:
                    omega = np.zeros((self.num_objectives,))
                ...
                self.agent.update(policy, optimizer, omega)
                if self.use_memory:
                    if rewards[self.edge_direction] >= optimal_rewards:
                        optimal_rewards = rewards[self.edge_direction]
            ...
            for i in range(self.pareto_front_size):
                self.update_ParetoFront(policy, optimizer)
                if i % 10 == 0:
                    current_hv = self.HV()
                    current_sp = self.SP()
